...
// recorded from tt.horner.tj on 2026-09-02, times rebased to now on each tick
...
function tripsFor(slot: Slot, nowMs: number): Trip[] {
  const seeds = SEEDS[slot.stopId] ?? [];
  const base = Math.floor(nowMs / 1000);
  const trips = seeds.flatMap((seed, s) =>
  ...
  return trips.sort((a, b) => a.arrivalTime - b.arrivalTime);
}

export const fixtureSource: TransitSource = {
  subscribe(slot, onTrips) {
    const anchor = Date.now();
    onTrips(tripsFor(slot, anchor));
    const timer = setInterval(() => onTrips(tripsFor(slot, anchor)), 15_000);
