...
// late reads as a warning; early sits with the clock time, since the green is reserved for the live marker
export function Lateness({ value }: { value: Value }) {
  const text = latenessText(value);
  return (
    <span className={`w-[4.5rem] text-right font-mono text-body whitespace-nowrap ${text?.late ? 'text-warn' : 'text-near'}`}>
      {text ? (
        <>
